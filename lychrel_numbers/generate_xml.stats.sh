...
function gen_xml() {
 cat log/${2}.${1}.stats.sh.log log/${2}.${1}.stats.sh.log.2 |\
	sed -f sed/stats.sh.log.sed |\
	grep -f grep/stats.sh.log.grep |\
	gawk -f awk/xml.awk -f awk/generate_xml.stats.awk\
	>> xml/${1}.stats.sh.log.xml
}

function gen_xml2() {
 echo "" | gawk -v file_name="log/*.${1}.stats.sh.log.2" -v algorithm="$1" -f awk/xml.awk\
		-v xsl_file="xsl/compare_stats.xsl" -f awk/begin.awk\
		> xml/${1}.stats.sh.log.xml
 for PRG_LANG in $2; do
	echo " XML file generation (raw data) for $PRG_LANG."
	gen_xml $1 "$PRG_LANG"
 done
 cat log/${1}.stats.sh.log.2 | gawk -f awk/xml.awk -f awk/end.awk >> xml/${1}.stats.sh.log.xml
}
